        ...
        <div class="form-group mt-4">
            ...
            <select class="form-control multi-select" name="jenis_beasiswa[]" id="option_jenis_beasiswa" multiple>
                <option value="Partial"
                    {{ in_array('Partial', request('jenis_beasiswa', [])) ? 'selected' : '' }}>Partial</option>
                <option value="Full" {{ in_array('Full', request('jenis_beasiswa', [])) ? 'selected' : '' }}>
                    Full</option>
            </select>
        </div>
                    ...
                    <div class="col-lg-6 mb-4">
                        <div class="card h-100 shadow-sm">
                            <div class="card-body">
                                <div class="mb-2">
                                    <!-- Tingkat Studi -->
                                    @foreach ($article->tingkatStudi as $tingkat)
                                ...
                                @foreach ((array) $article->jenis_beasiswa as $jenis)
                                    <p>{{ $jenis == 'full' ? 'Beasiswa di Biayai Penuh' : 'Beasiswa Di Biayai Sebagian' }}
                                    </p>
                                @endforeach
...
$(document).ready(function() {
    ...
    // Handle AJAX form submission for filtering
    $('#filterForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                $('#articleList').html(renderArticles(response.data));
                // Handle messages or other parts of the response if needed
                if (response.message) {
    ...
    // Handle AJAX form submission for sorting
    ...
    function renderArticles(data) {
        ...
        $.each(data, function(index, article) {
            ...
            html += '<div class="mb-2">';
            $.each(article.tingkat_studi, function(index, tingkat) {
                html += '<span class="badge bg-primary">' + tingkat.nama + '</span>';
            });
            $.each(article.negara, function(index, negara) {
                html += '<span class="badge bg-success">' + negara.nama + '</span>';
            });
            html += '</div>';
            $.each(article.jenis_beasiswa, function(index, jenis) {
                html += '<p>' + (jenis === 'full' ? 'Beasiswa di Biayai Penuh' : 'Beasiswa Di Biayai Sebagian') + '</p>';
            });
            html += '<h5 class="card-title text-center mt-3">' + article.judul + '</h5>';
            html += '<div class="bg-gray p-3 mt-3 rounded">';
            html += '<div class="d-flex justify-content-between">';
            html += '<p><strong>Tanggal Registrasi:</strong></p>';
            html += '<p class="date-registrasi">' + article.tanggal_registrasi + '</p>';
            html += '</div>';
            html += '<div class="d-flex justify-content-between">';
            html += '<p><strong>Deadline:</strong></p>';
            html += '<p class="date-deadline text-danger">' + article.deadline + '</p>';
            html += '</div>';
            html += '</div>';
            html += '<a href="/detail/' + article.id + '#DetailKalenderBeasiswa" class="btn btn-primary mt-3">Detail Beasiswa</a>';
            html += '</div>';
            html += '</div>';
            html += '</div>';
        });
