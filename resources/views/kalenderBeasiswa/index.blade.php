...
<div class="card-body">
    ...
    <table id="example2" class="table table-bordered table-hover">
        ...
        @foreach ($data as $item)
            ...
            <td>
                <form action="{{ route('kalenderBeasiswa.destroy', $item->id) }}" method="POST">
                @csrf
                @method('DELETE')
            ...
            <div class="modal-dialog modal-lg">
                ...
                <form action="{{ route('kalenderBeasiswa.update', $item->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="modal-body">
                    <div class="form-group">
                        ...
                        <select class="form-control" name="id_tingkat_studi[]" id="option_tingkat_studi_{{ $item->id }}" multiple>
                            @foreach ($tingkatStudi as $i)
                            <option value="{{ $i->id }}" {{ in_array($i->id, $item->tingkatStudi->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $i->nama }}</option>
                            @endforeach
                        </select>
                        </div>
